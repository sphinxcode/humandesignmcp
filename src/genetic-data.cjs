...
function enrichActivation(activation, planetName) {
  ...
  // Get gate name and I Ching glyph
  const gateName = GATE_NAMES[gate] || null;
  const iChingGlyph = I_CHING_GLYPHS[gate] || null;

  // Get gate enrichment data
  const gateDescription = GATE_DESCRIPTIONS[gate] || null;
  ...
  const harmonicDescription = harmonicPartner ? GATE_DESCRIPTIONS[harmonicPartner] : null;

  // Get electromagnetic partner (channel partner)
  let electromagnetPartner = ELECTROMAGNETIC_GATES[gate] || null;
  let electromagnetDescription = null;

  // Handle cases where gates have multiple electromagnetic partners (gates 10, 20, 34, 57)
  if (Array.isArray(electromagnetPartner)) {
    // For gates with multiple channel partners, we'll list all of them
    electromagnetDescription = electromagnetPartner.map(p => GATE_DESCRIPTIONS[p] || null);
    electromagnetPartner = electromagnetPartner; // Keep as array
  } else if (electromagnetPartner) {
    electromagnetDescription = GATE_DESCRIPTIONS[electromagnetPartner];
  }
